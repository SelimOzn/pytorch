...
#torch.save(model, FILE)    #Tensor, model, parametre dict'i arg olabilir. Python'ın pickle'ını kullanır ve serialize ederek save eder. Kaydettik artık yorum olarak kalabilir.

#Bu kaydetmek için lazy yoldu.
...
for param in model.parameters():
    print(param) #Bu ve alttaki modelin parametreleri aynı.
...
#Save on the GPU, load on the CPU
...
#Save on the GPU, load on the GPU
device = torch.device("cuda")
model.to(device)
